# 关卡美术配置与本地验收（含 v18 历史发布快照）

> 本文的版本、commit、素材与测试计数按上一轮 Web 发布 checkout 冻结，不代表 canonical World Studio / Godot 第一阶段当前状态。当前状态以 [`PHASE_ONE_REPORT.md`](PHASE_ONE_REPORT.md) 和重新生成的同 hash 本地证据为准。当前流程只做本地仓库开发、Git 多机同步、localhost Chrome 验收和 Godot 4.7.1 本地导出；不部署 Sites、不复验公开 URL。历史 Sites “通过”仅作归档，不能证明当前或未来发布。
...
7. 构建通过后在 localhost 重新确认新入口、素材请求、编辑保存和试玩；当前不执行 Sites 发布。
8. 只有上述证据全部属于当前 checkout/工作树指纹和本地构建，才可把本轮本地美术验收标记为完成。
...
### v18 历史 Sites 发布与线上复验（归档，不是当前门禁）
