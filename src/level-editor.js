...
export function createLevelEditor({ root, sourceLevels, onPlay, onSavedLevelsChange }) {
  ...
  function drawObject(object) {
    ...
    if (object.type === "platform") {
      ...
      ctx.fill();
    } else if (["windZone", "checkpoint", "rotationTrigger"].includes(object.type)) {
    ...
    if (view.zoom >= 0.34 && object.type !== "backgroundSeed") {
      const labelX = object.type === "platform" ? x + 7 : x + 20 / view.zoom;
      const labelY = object.type === "platform" ? y + 18 / view.zoom : y - 10 / view.zoom;
  ...
  return { open, close, getSavedDocuments: () => clone(savedDocuments) };
}
